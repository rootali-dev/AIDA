//! ai-ida-ebpf/src/parsers/ipv6.rs
...
pub fn parse_ipv6(cursor: &mut Cursor) -> Result<L3Ipv6Info, ()> {
    ...
    // پیمایش کراندار Extension Headerها (حداکثر ۴ اکستنشن)
    for _ in 0..4 {
        match next_proto {
            // پروتکل‌های اکستنشن استاندارد IPv6
            0 | 43 | 60 => {
                let ext = cursor.read::<Ipv6ExtHdr>()?;
                next_proto = unsafe { (*ext).next_header };
                let ext_len = ((unsafe { (*ext).hdr_ext_len } as usize) + 1) * 8;
                // ما ۲ بایت خوانده‌ایم، باقیمانده را پرش می‌کنیم
                if ext_len > 2 {
                    cursor.advance(ext_len - 2)?;
                }
            }
            44 => {
                // Fragment Header (طول ثابت ۸ بایت)
                let ext = cursor.read::<Ipv6ExtHdr>()?;
                next_proto = unsafe { (*ext).next_header };
                cursor.advance(6)?;
            }
