...
برای جلوگیری از Paddingهای ناخواسته کامپایلر و تضمین همخوانی کامل بین **Rust (کرنل)** و **Go (یوزراسپیس)**، این ساختار داده مبنای تمام تبادلات قرار می‌گیرد.

## ساختار C / Rust (`repr(C, packed)`)
...
1. ساختار باید همواره ضریبی از ۸ یا ۴ بایت باشد.
    
2. هیچ اشاره‌گری (Pointer) نباید در ساختار قرار گیرد.
    
3. در زمان کامپایل، تست assert_eq!(core::mem::size_of::<PacketTelemetry>(), 24); باید در Rust پاس شود.