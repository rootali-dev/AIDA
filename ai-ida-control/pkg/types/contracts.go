...
// FlowPacketMeta آینه مستقیم ساختار FlowPacketMeta در ai-ida-common/src/telemetry.rs
// حجم کل: دقیقا ۲۴ بایت | تراز: ۸ بایت
...
// LpmIpv4Key کلید مپ LPM_TRIE برای بلک‌لیست آی‌پی و ساب‌نت
type LpmIpv4Key struct {
	PrefixLen uint32
	Addr      [4]byte
}

// RateLimitState داده ذخیره شده در مپ ریت‌لیمیتر
type RateLimitState struct {
	LastUpdateNS uint64
	Tokens       uint64
}

// NewLpmKey ساخت یک کلید CIDR برای LPM_TRIE
func NewLpmKey(cidrStr string) (LpmIpv4Key, error) {
	ip, ipNet, err := net.ParseCIDR(cidrStr)
	if err != nil {
		// اگر تک IP وارد شده باشد، به صورت 32/ در نظر می‌گیریم
...
// UnmarshalBinary تجزیه باینری امن ۲۴ بایت تله‌متری
func (m *FlowPacketMeta) UnmarshalBinary(data []byte) error {
	...
	// آی‌پی‌ها در شبکه Big-Endian هستند
